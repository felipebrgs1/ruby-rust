...
- Default daemon preload: `json,erb,pathname,fileutils,time,date,digest,base64,ostruct,set,stringio,logger,socket` — só os módulos baratos (~22ms); os pesados (yaml, uri, net/http, csv — ~43ms) são **lazy**: o child carrega no require on demand (custo único no child, CoW — não polui o daemon) e o cold do daemon cai ~43ms. `--preload`/`CALISTO_PRELOAD` restauram a lista completa se o caso de uso pedir.
