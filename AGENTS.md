...
rubies sem .so, ex.: 3.4.4) e **Fase M completa** (memória/CoW: `[run]
...
env_blob — sem mudança de protocolo, os dois daemons (embutido e legado)
aplicam pós-Bundler.setup; `-c` compila sem executar e pula requires como
...
não-fazer documentado). Ciclo L–R fechado; próximos candidatos: APIs
nativas novas (`calisto.*` além de hash/sqlite), degraus reais com o
instalado, snapshot gated se o cenário de privilégio mudar.
...
**`calisto run -e 'code'`** (Fase G): op **`EVAL`** no daemon (mesmo formato do RUN: cwd, env_blob, code, args) — o child roda com semântica exata de `ruby -e`: no daemon EMBUTIDO via cadeia iseq do CLI (`rb_parser_new` → compile `"-e"` → `rb_iseq_new_main` → `rb_iseq_eval_main` — backtrace só `-e:N`, sem frames de eval); no legado via `eval(code, TOPLEVEL_BINDING, "-e", 1)` — com `$0 = "-e"` (sem DATA): paridade exata com `ruby -e` em $0/ARGV/backtrace (`-e:1`)/exit codes. Múltiplos `-e` são concatenados com `"\n"` no cliente (como o ruby; `__LINE__` segue a concatenação). Cold: `ruby -rbundler/setup -e code args` — paridade cold/warm é invariante coberta no `exec.rs`. O child ganhou `child_enter` (bootstrap comum: traps default, dup stdio, hygiene de fds, chdir, ENV.replace, `bundler/setup`, CALISTO_LOAD_PATH + `$calisto_native_dir` — Fase P) compartilhado por RUN/EVAL.
...
- `daemon.rs` — socket reuse, stale-socket recovery, orphan kill on client death (checks `/proc/<pid>`), signal exit codes (`SIGKILL` → 137), concurrent runs, pipeline non-hang; **Fase L**: daemon embutido (pidfile → `/proc/<pid>/exe` == binário calisto) e fallback legado via `CALISTO_NO_EMBED=1` (exe == `bin/ruby`).
